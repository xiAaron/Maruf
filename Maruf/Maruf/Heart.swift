  ...
  class Heart: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate, ProjectAbout {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if (row == 0) {
            ...
            performSegue(withIdentifier: "Heart-Tops", sender: self)
        }
        else if (row == 2) {
            performSegue(withIdentifier: "Heart-Povertunity", sender: self)
        }
        else if (row == 3) {
            performSegue(withIdentifier: "Heart-HalfOfFaith", sender: self)
        }
        else if (row == 4) {
            performSegue(withIdentifier: "Heart-CommunityGardens", sender: self)
        }
        else if (row == 5) {
            performSegue(withIdentifier: "Heart-Center", sender: self)
        }
    }
    ...
    @IBAction func projectsReturn(_ sender: UIButton) {
        ...
        let OKAction = UIAlertAction(title: "Return", style: UIAlertActionStyle.default, handler: {
            (_)in
            self.performSegue(withIdentifier: "HeartunwindToMenu", sender: self)
        })
    ...
    let pickerDataSource = ["HEART", "TOPS", "Povertunity", "Half of Faith", "Community Garden", "Center"];
    ...
    override func viewDidLoad() {
        ...
        getVideo(videoCode: "Ao17ttK_P2E")
    }
